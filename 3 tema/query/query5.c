...
int main(int argc, char *argv[]) {
    ...
    if (db){
        ...
        for (int i = 0; i < size; ++i){ // process all the student records in database
            ...
            int sNameLength = 0;
            int sSurnameLength = 0;
            while(s.name[sNameLength]!='\0') sNameLength++;
            while(s.surname[sSurnameLength]!='\0') sSurnameLength++;
            if(sNameLength > sSurnameLength){ // *** first filter, conditions on the student
